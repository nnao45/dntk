...
fn main() {
    ...
    let mut before_printed_statement_len = 0;
    let mut currnet_cur_pos = 0;
    loop {
        ...
        if r > 0 {
            ...
            match char_scan(input_char) {
                ...
                Some(util::ASCII_CODE_DELETE) => {
                    if currnet_cur_pos > 0 {
                        currnet_cur_pos -= 1;
                        &input_vec.remove(currnet_cur_pos);
                    }
                },
                ...
                Some(util::ASCII_CODE_SQUARERIGHT) => {
                    if currnet_cur_pos < before_printed_statement_len {
                        currnet_cur_pos += 1;
                    }
                },
                Some(i) => {
                    currnet_cur_pos += 1;
                    &input_vec.insert(currnet_cur_pos-1, i);
                    },
            }
            ...
            match bc!(format!("{}", p2)) {
                Ok(p4) => {
                    ...
                    before_printed_statement_len = p2.to_string().len();
                    before_printed_len = p1.to_string().len() + before_printed_statement_len + p3.to_string().len() + before_printed_result_len;
                    let pos_differnce = before_printed_statement_len - currnet_cur_pos;
                    print!("{}{}{}{}{}{}", util::COLOR_CYAN_HEADER, p1, p2, p3, &p4, util::COLOR_PLAIN_HEADER);
                    print!("{}{}{}", util::CURSOR_MOVE_ES_HEAD, (p3.to_string().len() + before_printed_result_len + pos_differnce), util::CURSOR_MOVE_ES_BACK);
                    },
                _ => {
                    before_printed_statement_len = p2.to_string().len();
                    before_printed_len = p1.to_string().len() +  before_printed_statement_len + p3.to_string().len() + before_printed_result_len;
