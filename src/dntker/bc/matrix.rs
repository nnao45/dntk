...
impl MatrixParser {
    fn new(_exec: &super::BcExecuter, tokens: Vec<MatrixToken>) -> Self {
        Self {
            tokens,
            position: 0,
        }
    ...
    fn parse_primary(&mut self) -> Result<MatrixValue, MatrixParseError> {
        match self.next() {
            ...
            Some(MatrixToken::Number(value)) => Ok(MatrixValue::Scalar(value)),
        ...
        while !remaining.is_empty() {
            ...
            match ch {
                '[' => {
                    ...
                    let matrix = super::BcExecuter::parse_matrix(literal)
                        .map_err(MatrixParseError::from_bc_error)?;
                ...
                '0'..='9' | '.' => {
                    ...
                    let number = exec.promote_precision(number);
                    tokens.push(MatrixToken::Number(number));
                    remaining = rest;
                }
                _ => {
                    if ch.is_alphabetic() {
...
impl MatrixValue {
    fn add(self, other: MatrixValue) -> Result<Self, MatrixParseError> {
        match (self, other) {
            ...
            _ => Err(MatrixParseError::invalid(
                "matrix addition requires two matrices",
            )),
        }
    }
    ...
    fn sub(self, other: MatrixValue) -> Result<Self, MatrixParseError> {
        match (self, other) {
            ...
            _ => Err(MatrixParseError::invalid(
                "matrix subtraction requires two matrices",
            )),
        }
    }
    ...
    fn div(self, other: MatrixValue) -> Result<Self, MatrixParseError> {
        match (self, other) {
            (MatrixValue::Matrix(matrix), MatrixValue::Scalar(scalar)) => {
                ...
                Ok(MatrixValue::Matrix(divide_matrix(matrix, &scalar)))
            }
            (MatrixValue::Scalar(lhs), MatrixValue::Scalar(rhs)) => {
                if rhs.is_zero() {
                    return Err(MatrixParseError::invalid("matrix scalar division by zero"));
    ...
    fn negate(self) -> Self {
        match self {
            MatrixValue::Matrix(matrix) => {
                MatrixValue::Matrix(scale_matrix(matrix, &Decimal::from(-1)))
            }
            MatrixValue::Scalar(value) => MatrixValue::Scalar(-value),
        }
    }
}

fn scale_matrix(mut matrix: Vec<Vec<Decimal>>, scalar: &Decimal) -> Vec<Vec<Decimal>> {
...
fn divide_matrix(mut matrix: Vec<Vec<Decimal>>, scalar: &Decimal) -> Vec<Vec<Decimal>> {
    for row in &mut matrix {
        for value in row {
            *value /= scalar.clone();
        }
